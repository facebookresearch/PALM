...
from models.utils import scale_anything, get_activation, cleanup, chunk_batch_dim0
...
class BaseImplicitGeometry(BaseModel):
    ...
    def prepare_bbox(self, bbox):
        ...
        self.center = c
        self.scale = s
        ## note: scale here is full length of the max dimension; not the half length
        self.bbox = bbox

    def forward_level(self, points):
        raise NotImplementedError

    def isosurface_(self, vmin, vmax, cond):
        def batch_func(x):
            x = torch.stack([
            ...
            rv = self.forward_level(x, cond).cpu()
            cleanup()
            return rv
        level = chunk_batch_dim0(batch_func, self.config.isosurface.chunk, True, self.helper.grid_vertices())
...
class VolumeSDF(BaseImplicitGeometry):
    def setup(self):
        ...
        # in_dim = 3 + 45
        # network = get_mlp(in_dim, self.n_output_dims, self.config.mlp_network_config)
    ...
    def forward(self, points, cond, with_grad=True, with_feature=True, with_laplace=False):
        if points.numel() == 0:
            ret = [
                torch.empty(0, dtype=torch.float32).cuda()
            ]  # empty SDF
            if with_grad:
                ret.append(
                    torch.empty(0, 3, dtype=torch.float32).cuda()
                )  # empty gradient
            if with_feature:
                ret.append(
                    torch.empty(
                        0, self.n_output_dims, dtype=torch.float32).cuda()
                )  # empty feature
    ...
    def forward_level(self, points, cond):
