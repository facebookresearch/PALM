...
class LearnedLaplaceDensity(Density):  # alpha * Laplace(loc=0, scale=beta).cdf(-sdf)
    def setup(self):
        super().setup()
        self.beta_min = torch.tensor(self.config.get('beta_min', 0.0001))
...
class ScheduledLaplaceDensity(Density):  # alpha * Laplace(loc=0, scale=beta).cdf(-sdf)
    def setup(self):
        super().setup()
        self.beta0 = torch.tensor(self.config.get('beta0', 0.1))
        self.beta1 = torch.tensor(self.config.get('beta1', 0.001))
        self.t = 0.0
