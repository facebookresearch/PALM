...
class DummyNonRigidDeformer(BaseNonRigidDeformer):
    ## just return identity tfs
    ## points are returned without modifications
    def setup(self):
        pass

    def forward(self, points, cond, *args, with_jac=False):
        J_inv = torch.eye(points.shape[-1], device=points.device).unsqueeze(0).repeat(
            points.shape[0]*points.shape[1]*points.shape[2], 1, 1
        ).view(points.shape[0], points.shape[1], points.shape[2], 3, 3)
        return points, J_inv
