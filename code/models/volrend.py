...
from models.utils import chunk_batch, chunk_batch_original
...
def rendering(
    ...
    if rgb_sigma_fn is not None:
        ...
        if t_starts.shape[0] != 0:
            if chunk_size is None:
                sdfs, rgbs, alphas = rgb_alpha_fn(t_starts, t_ends, ray_indices, img_indices)
            else:
                sdfs, rgbs, alphas = chunk_batch_original(
                    rgb_alpha_fn,
                    chunk_size,
                    False,
                    t_starts,
...
def rendering_with_normals_sdf(
    ...
    if rgb_sigma_fn is not None:
        ...
        if t_starts.shape[0] != 0:
            (
                ...
                sdf_grad,
                laplace,
            ) = rgb_alpha_fn(t_starts, t_ends, ray_indices, img_indices)
        else:
        ...
        assert (
            normals_smpl.shape[-1] == 3
        ), "normals_smpl must have 3 channels, got {}".format(normals_smpl.shape)
        assert (
            normals_world.shape[-1] == 3
        ), "normals_world must have 3 channels, got {}".format(normals_world.shape)
        assert (
            alphas.shape == t_starts.shape
        ), "alphas must have shape of (N,)! Got {}".format(alphas.shape)
        assert (
        ...
        assert sdf_grad.shape[-1] == 3, "sdf_grad must have 3 channels, got {}".format(
            sdf_grad.shape
        )
        assert (
    ...
    return colors, fg_normal, normals, opacities, depths, extras
...
def rendering_with_normals_mats_sdf(
    ...
    albedo = albedo +  (1 - opacities) * torch.tensor(
        [1.0, 1.0, 1.0], device=albedo.device
