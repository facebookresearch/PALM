...
class FlatPoseEncoder(nn.Module):
    """Dummy pose encoder."""
    def __init__(self, config):
        super().__init__()
        self.lin1 = nn.Linear(48, config.pose_dim)
        # self.lin1 = nn.Linear(48, 12)

    def forward(self, aa, Jtrs):
        # from common.rot import rotation_matrix_to_angle_axis
        # aa = rotation_matrix_to_angle_axis(rots.view(16, 3, 3)).reshape(1, -1)[:, 3:]
        # features = self.lin1(aa[:, 3:])
        aa[:, :3] *= 0
        features = self.lin1(aa)
        return features
