...
class TemporalOccGridEstimator(AbstractEstimator):
    ...
    def sampling(
        ...
        t_idx = math.floor(t_idx * self.levels) ## 0
    ...
    def update(
        ...
        # for lvl, indices in enumerate(lvl_indices):
        lvl = 0 ## this increases as training goes; but it might be 0 all the time because I disabled TempOcc
        ...
        x = self.aabbs[lvl, :3] + x * (self.aabbs[lvl, 3:] - self.aabbs[lvl, :3]) ## sample points in world coordinate; NOTE: aabb is not updated.
        occs = occ_eval_fn(x).squeeze(-1) ## evaluate occ
        # # ema update
        cell_ids = lvl * self.cells_per_lvl + indices
        occs = F.max_pool3d(
            occs[cell_ids].reshape(1, 1, *self.resolution),
            kernel_size=3,
            stride=1,
            padding=1,
        )[0, 0].reshape(-1)

        self.binaries[lvl : lvl + 1] = (occs > 0).view(
