...
class VanillaFrequency(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.x_scale = config.get("x_scale", 1.0)
        self.x_offset = config.get("x_offset", 0.0)
...
class ProgressiveBandHashGrid(nn.Module):
    def __init__(self, in_channels, config):
        ...
        self.encoding = tcnn.Encoding(
            in_channels, encoding_config, dtype=torch.float32
        )
    ...
    def forward(self, x):
        ...
        ## point features from hashgrid
        # (Pdb) enc.shape
        # torch.Size([195257, 32])

        ## This mask decide the grainuaitiy of the features (which level in hashgrid to use)
        # (Pdb) self.mask.shape
        # torch.Size([32])
        # (Pdb) self.mask
        enc = enc * self.mask.cuda()
        return enc
...
class IntegratedDirectionalEncoding(nn.Module):
    ...
    def forward(self, xyz, kappa_inv):
        ...
        sph_harms = vmxy * torch.einsum(
            "pi,ij->pj", vmz, self.mat
        )  # TODO: we assume input xyz is n_pts x 3, which does not consider batch size
...
class CompositeEncoding(nn.Module):
    def __init__(self, encoding, include_xyz=False, xyz_scale=1.0, xyz_offset=0.0):
        ...
        if hasattr(self.encoding, "n_input_dims") and hasattr(
...
def get_encoding(n_input_dims, config):
    ...
    if config.otype == "Identity":
        ...
        encoding = tcnn.Encoding(
            n_input_dims, config_to_primitive(config), dtype=torch.float32
        )
