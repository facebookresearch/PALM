...
class SaverMixin:
    ...
    DEFAULT_GRAYSCALE_KWARGS = {"data_range": (0.6, 1.5), "cmap": "jet"}
    ...
    def save_image_grid(self, filename, imgs):
        ...
        if img.dtype == np.uint8:
            ...
            Image.fromarray(img).save(self.get_save_path(filename))
            # cv2.imwrite(self.get_save_path(filename), img)
        elif img.dtype == np.float32:
            import os.path as op
            import os
            exp_id = self.get_save_path(filename).split('/')[1]
            out_p = self.get_save_path(filename).replace(exp_id+'/save', exp_id + "/" + exp_id + '/envs')
            it = int(op.basename(out_p).split('-')[0].replace('it', ''))
            basename = f'it{it:06}.exr'
            out_p = op.join(op.dirname(out_p), basename)
            os.makedirs(op.dirname(out_p), exist_ok=True)
            pyexr.write(out_p, img)
        else:
    ...
    def save_image(self, filename, img):
        ...
        Image.fromarray(img, self.get_save_path(filename))
